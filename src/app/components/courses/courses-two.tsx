...
export default function CoursesTwo({item}: CourseItemProps) {
  return (
            ...
            <div className="md:flex">
                ...
                <div className="p-6 w-full">
                    <div className="flex mb-3">
                        <span className="text-slate-400 text-sm flex items-center"><FiBook className="text-slate-900 dark:text-white size-[14px] me-1"/>{item.lessons} Уроков</span>
                        <span className="text-slate-400 text-sm flex items-center ms-3"><FiUsers className="text-slate-900 dark:text-white size-[14px] me-1"/>{item.students} Студентов</span>
                    </div>
                    ...
                    <div className="flex justify-between md:mt-12 mt-3">
                        {!item.hideTeacher && (
                          <span className="flex items-center">
                            <Image src={item.user} width={32} height={32} className="size-8 rounded-full shadow shadow-slate-100 dark:shadow-slate-800 me-2" alt=""/> 
                            <Link href="" className="hover:text-violet-600 font-medium">{item.name}</Link>
                          </span>
                        )}
                        <Link href={`/course-detail-two/${item.id}`} className="h-8 px-3 tracking-wide inline-flex items-center justify-center font-medium rounded-md bg-violet-600/10 hover:bg-violet-600 text-violet-600 hover:text-white text-sm">Подробнее <i className="mdi mdi-arrow-right align-middle ms-1"></i></Link>
                    </div>
                </div>
            </div>
        </div>
